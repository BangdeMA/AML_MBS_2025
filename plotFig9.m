...
A = load('CombTherapyOS.txt');
B = load('CombTherapyOSB.txt');
C = load('CombTherapyOSC.txt');
OS_simulationA  = A(:,2);
OS_simulationB  = B(:,2);
OS_simulationC  = C(:,2);
%% read clinical survival data and simulation survival data
...
legend('No Therapy','box','off')
set(gca,'box','on');
set(gca,'linewidth',k);
text(-10,1,'A','fontweight','bold','FontSize',30);
...
xlabel('Time (months)')
ylabel('Survival probability')
legend('Chemotherapy','box','off')
set(gca,'box','on');
set(gca,'linewidth',k);
text(-10,1,'B','fontweight','bold','FontSize',30);
...
xlabel('Time (months)')
ylabel('Survival probability')
legend('Targeted Therapy','box','off')
set(gca,'box','on');
set(gca,'linewidth',k);
text(-10,1,'C','fontweight','bold','FontSize',30);
%%
nColor = 256;
...
% scatter(X, Y, density_size, density, 'filled');
